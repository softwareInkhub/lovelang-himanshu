// LoveLang Image URLs organized by category and product type

export const LOVELANG_IMAGES = {
  // Main banner and shop images
  banners: {
    shopAllBanner: "https://lovelang.in/cdn/shop/files/shop_all_banner_1.jpg?v=1747823477&width=1000",
    // New Dynamic Banner Images
    skinificationBanner: "https://lovelang.in/cdn/shop/files/LLW_Homepage_Banner_Mob_4.jpg?v=1748324122&width=1000",
    fruitPoweredBanner: "https://lovelang.in/cdn/shop/files/LLW_Homepage_Banner_Mob_15.jpg?v=1748516921&width=1000", 
    loveLanguageBanner: "https://lovelang.in/cdn/shop/files/LLW_Homepage_Banner_Mob_3.jpg?v=1748322770&width=1000",
    avocadoCollectionBanner: "https://lovelang.in/cdn/shop/files/Avocado_Collection_2.jpg?v=1747818934&width=1000",
    hotSpotBanner: "https://lovelang.in/cdn/shop/files/LLW_Hot_Spot_Mob_61a3f796-8269-48dd-bc3c-40cdf6afc84a.jpg?v=1747810531&width=1200",
    testimonialBanner: "https://lovelang.in/cdn/shop/files/3.jpg?v=1746700113&width=1000",
    landscapeAsset: "https://lovelang.in/cdn/shop/files/LLW_Landscape_Asset_03a321ba-0003-472b-919b-50b42b2e0b28.png?v=1746699398&width=800",
  },

  // Product Collection Images
  collections: {
    avocado: "https://lovelang.in/cdn/shop/collections/Avocado_shop.jpg?v=1746785628&width=240",
    mango: "https://lovelang.in/cdn/shop/collections/mango_shop_4.jpg?v=1746785590&width=240",
    untitledDesign: "https://lovelang.in/cdn/shop/collections/Untitled_design_4.jpg?v=1746785613&width=240",
  },

  // Product Images by Category
  products: {
    // Mango Collection (MC - Mango Collection)
    mango: {
      primary: [
        "https://lovelang.in/cdn/shop/files/MC1_388dd51f-bf3c-49a0-adf8-866cfaecd030.jpg?v=1747815833&width=1000",
        "https://lovelang.in/cdn/shop/files/MC2_e6c27907-cc33-452c-9e25-fb54887cfe43.jpg?v=1747815833&width=1000",
      ],
      featured: "https://lovelang.in/cdn/shop/files/MC2_e6c27907-cc33-452c-9e25-fb54887cfe43.jpg?v=1747815833&width=1000",
      masks: [
        "https://lovelang.in/cdn/shop/files/MM1_4.jpg?v=1747808804&width=700",
        "https://lovelang.in/cdn/shop/files/MM2.jpg?v=1747808804&width=1000",
      ],
      shampoo: [
        "https://lovelang.in/cdn/shop/files/MS1_1_fd444c24-7c59-4c69-b4d6-c3c38a6743be.jpg?v=1748252455&width=700",
        "https://lovelang.in/cdn/shop/files/MS2_411d85c7-75c1-4eac-8c76-932508e5c89a.jpg?v=1748252455&width=1000",
      ],
      icon: "https://lovelang.in/cdn/shop/files/MS_png.png?v=1746788190&width=240",
    },

    // Peach Collection (PC - Peach Collection)
    peach: {
      primary: [
        "https://lovelang.in/cdn/shop/files/PC1_4ceb978c-a312-41c3-af67-15d46818bf39.jpg?v=1747112435&width=1000",
        "https://lovelang.in/cdn/shop/files/PC2_105826fe-ee21-44c9-a2e9-6e4f9a55fed6.jpg?v=1747652218&width=1000",
      ],
      featured: "https://lovelang.in/cdn/shop/files/PC2_105826fe-ee21-44c9-a2e9-6e4f9a55fed6.jpg?v=1747652218&width=1000",
      masks: [
        "https://lovelang.in/cdn/shop/files/PM1_4f402b5e-1110-40df-b24e-01e409be6553.jpg?v=1748252426&width=700",
        "https://lovelang.in/cdn/shop/files/PM1_9a1fa6a9-a651-479b-a6b1-830f723e1047.jpg?v=1747112479&width=700",
        "https://lovelang.in/cdn/shop/files/PM2.jpg?v=1747112479&width=1000",
      ],
      shampoo: [
        "https://lovelang.in/cdn/shop/files/PS2.jpg?v=1748252426&width=1000",
      ],
      icon: "https://lovelang.in/cdn/shop/files/PS_png.png?v=1746788307&width=240",
    },

    // Avocado Collection (AC - Avocado Collection)
    avocado: {
      primary: [
        "https://lovelang.in/cdn/shop/files/AC1_f418ed94-58e6-4088-a27c-63cc01c4b799.jpg?v=1747112377&width=1000",
        "https://lovelang.in/cdn/shop/files/AC2_bbd43ee2-b7ff-46a5-936f-a323e575c765.jpg?v=1747652205&width=1000",
      ],
      featured: "https://lovelang.in/cdn/shop/files/AC2_bbd43ee2-b7ff-46a5-936f-a323e575c765.jpg?v=1747652205&width=1000",
      masks: [
        "https://lovelang.in/cdn/shop/files/AM1_858f72e1-6aba-49ec-90a2-9dbd4164d6e9.jpg?v=1747112561&width=700",
        "https://lovelang.in/cdn/shop/files/AM2.jpg?v=1747112561&width=1000",
      ],
      shampoo: [
        "https://lovelang.in/cdn/shop/files/AS1_4a592b06-dfd4-47a8-a000-66fe237670a0.jpg?v=1748252442&width=700",
        "https://lovelang.in/cdn/shop/files/AS2.jpg?v=1748252442&width=1000",
      ],
      icon: "https://lovelang.in/cdn/shop/files/AS_png.png?v=1746788875&width=240",
    },
  },

  // Brand/Quality Icons
  badges: {
    fdcaApproved: "https://lovelang.in/cdn/shop/files/FDCA_fae2d4b2-267d-4b6f-9e8f-b72013dfa427.png?v=1746600442&width=150",
    vitaminInfused: "https://lovelang.in/cdn/shop/files/Vitamin_3d5906de-be18-4316-952f-17b2af85367e.png?v=1746600442&width=150",
    vegan: "https://lovelang.in/cdn/shop/files/Vegan_1ad64ffd-103c-4934-8303-f27e5ca20f3c.png?v=1746600441&width=150",
    crueltyFree: "https://lovelang.in/cdn/shop/files/Cruelty_Free_309c646f-8778-40b3-ba48-80b7328630a6.png?v=1746600441&width=150",
  },

  // Testimonial/Review Images
  testimonials: [
    "https://lovelang.in/cdn/shop/files/unnamed.jpg?v=1747822142&width=192",
    "https://lovelang.in/cdn/shop/files/1000150620.jpg?v=1747821266&width=192",
    "https://lovelang.in/cdn/shop/files/1000150615.jpg?v=1747821481&width=192",
  ],
};

// Helper function to get optimized image URL with custom width
export function getOptimizedImageUrl(baseUrl: string, width: number = 800): string {
  // Remove existing width parameter and add new one
  const url = new URL(baseUrl);
  url.searchParams.set('width', width.toString());
  return url.toString();
}

// Example usage in components:
/*
import { LOVELANG_IMAGES, getOptimizedImageUrl } from '@/data/image-urls';

// Use in JSX:
<img 
  src={LOVELANG_IMAGES.products.avocado.primary[0]} 
  alt="Avocado Hair Care Product" 
/>

// Or with custom sizing:
<img 
  src={getOptimizedImageUrl(LOVELANG_IMAGES.products.mango.masks[0], 600)} 
  alt="Mango Hair Mask" 
/>
*/